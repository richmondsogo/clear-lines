...
import {
  ...
  PanelLeftClose, Plus, Search, Settings, Sun, Trash2, X
} from "lucide-react";
...
import type { Folder, Note, Preferences } from "./types";
...
const makeNote = (): Note => ({
  id: crypto.randomUUID(), title: "", body: "", folder: "inbox", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()
});
...
export default function App() {
  ...
  const visible = notes.filter(note => {
    ...
    const haystack = `${noteTitle(note)} ${cleanText(note.body)}`.toLowerCase();
  ...
  useEffect(() => saveData({ notes, preferences }), [notes, preferences]);
  useEffect(() => {
  ...
  function moveNote(folder: Folder) { updateNote({ folder }); if (folder === "trash") setView("trash"); }
...
function NoteCard({ note, selected, onClick }: { note: Note; selected: boolean; onClick(): void }) { return <button className={`note-card ${selected ? "selected" : ""}`} onClick={onClick}><div><strong>{noteTitle(note)}</strong><p>{cleanText(note.body) || "No additional text"}</p></div><time>{formatDate(note.updatedAt)}</time>{note.pinned && <span className="pin">•</span>}</button>; }
...
function Editor({ note, editorRef, onUpdate, onMove, onDelete }: { note: Note | null; editorRef: RefObject<HTMLDivElement>; onUpdate(patch: Partial<Note>): void; onMove(folder: Folder): void; onDelete(): void }) {
  ...
  return <article className="editor"><div className="editor-meta"><span>{formatDate(note.updatedAt)} · {cleanText(note.body).split(/\s+/).filter(Boolean).length} words</span><div className="menu-wrap"><button className="icon-button" onClick={() => setMenuOpen(!menuOpen)} aria-label="Note actions"><MoreHorizontal size={20}/></button>{menuOpen && <div className="note-menu"><button onClick={() => { onUpdate({ pinned: !note.pinned }); setMenuOpen(false); }}>{note.pinned ? "Unpin note" : "Pin note"}</button>{note.folder !== "archive" && <button onClick={() => { onMove("archive"); setMenuOpen(false); }}>Archive note</button>}{note.folder === "trash" ? <button className="danger" onClick={onDelete}>Delete forever</button> : <button className="danger" onClick={() => { onMove("trash"); setMenuOpen(false); }}>Move to trash</button>}</div>}</div></div><input id="note-title" value={note.title} placeholder="Untitled note" onChange={event => onUpdate({ title: event.target.value })} aria-label="Note title"/><TiptapEditor key={note.id} content={note.body} onUpdate={body => onUpdate({ body })}/></article>;
}
...
function SearchDialog({ query, setQuery, notes, onClose, onSelect }: { query: string; setQuery(v: string): void; notes: Note[]; onClose(): void; onSelect(note: Note): void }) { const input = useRef<HTMLInputElement>(null); useEffect(() => input.current?.focus(), []); const matches = notes.filter(note => `${noteTitle(note)} ${cleanText(note.body)}`.toLowerCase().includes(query.toLowerCase())).slice(0, 8); return <div className="dialog-backdrop" onMouseDown={onClose}><div className="search-dialog" onMouseDown={e => e.stopPropagation()}><div><Search size={19}/><input ref={input} value={query} onChange={e => setQuery(e.target.value)} placeholder="Search your notes…" aria-label="Search notes"/><button className="icon-button" onClick={onClose}><X size={18}/></button></div><section>{matches.length ? matches.map(note => <button key={note.id} onClick={() => onSelect(note)}><FileText size={17}/><span><strong>{noteTitle(note)}</strong><small>{cleanText(note.body) || "Empty note"}</small></span><time>{formatDate(note.updatedAt)}</time></button>) : <p>No matching notes.</p>}</section></div></div>; }
